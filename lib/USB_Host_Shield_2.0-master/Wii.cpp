...
WII::WII(BTD* p, bool pair) : BluetoothService(p)  // Pointer to USB class instance - mandatory
{
...
bool WII::getButtonPress(ButtonEnum b) {  // Return true when a button is pressed
    if (wiiUProControllerConnected)
        return (ButtonState & WII_PROCONTROLLER_BUTTONS[(uint8_t)b]);
    else
        return (ButtonState & WII_BUTTONS[(uint8_t)b]);
}

bool WII::getButtonClick(ButtonEnum b) {  // Only return true when a button is clicked
    uint32_t button;
    if (wiiUProControllerConnected)
        button = WII_PROCONTROLLER_BUTTONS[(uint8_t)b];
    else
        button = WII_BUTTONS[(uint8_t)b];
