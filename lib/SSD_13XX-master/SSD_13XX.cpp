...
void SSD_13XX::begin(bool avoidSPIinit) {
    ...
    *csportSet = cspinmask;
    ...
    pinMode(_cs, OUTPUT);
    if (!avoidSPIinit) {
		Serial.println("custom spi begin !!!");
		spiOled.begin(22,-1,21,-1);
	}
    //if (!avoidSPIinit) SPI.begin();
#if !defined(SPI_HAS_TRANSACTION)
    if (!avoidSPIinit) {
    ...
    writecommand_cont(CMD_GRAYSCALE);
    for (uint8_t i = 0; i < 32; i++) {
        writecommand_cont(SSD_GRAYTABLE[i]);
    }
#else
    ...
    writedata8_cont(SSD_SETVSL_A);
    writedata8_cont(SSD_SETVSL_B);
    writedata8_cont(SSD_SETVSL_C);
    writecommand_cont(CMD_CONTRASTABC);
    writedata8_cont(SSD_CONTRAST_A);
    writedata8_cont(SSD_CONTRAST_B);
    writedata8_cont(SSD_CONTRAST_C);
    setRegister_cont(CMD_MASTERCURRENT, SSD_MASTERCURRENT);
...
void SSD_13XX::setRotation(uint8_t m) {
    ...
    _cursorX = 0;
    _cursorY = 0;  //always reset cursor
    startTransaction();
    ...
    if (startAngle > endAngle) {
        ...
        for (x = xmin; x <= xmax; x++) {
            ...
            for (y = ymin; y <= ymax; y++) {
                ...
                y2 = y * y;
                if (
                    (x2 + y2 < or2 && x2 + y2 >= ir2) && ((y > 0 && startAngle < 180 && x <= y * sslope) || (y < 0 && startAngle > 180 && x >= y * sslope) || (y < 0 && startAngle <= 180) || (y == 0 && startAngle <= 180 && x < 0) || (y == 0 && startAngle == 0 && x > 0)) && ((y > 0 && endAngle < 180 && x >= y * eslope) || (y < 0 && endAngle > 180 && x <= y * eslope) || (y > 0 && endAngle >= 180) || (y == 0 && endAngle >= 180 && x < 0) || (y == 0 && startAngle == 0 && x > 0))) {
...
void SSD_13XX::_pushColors_cont(uint16_t data, uint32_t times) {
    enableDataStream();
    while (times--) {
        spiOled.transfer(data >> 8);
        spiOled.transfer(data);
    }
}
#endif
/* -------------------------------------------------------------------------------------------
